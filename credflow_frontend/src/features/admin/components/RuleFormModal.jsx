...
const RuleFormModal = ({ isOpen, onClose, onSave, rule, templates = [] }) => {
    ...
    const handleChange = (key, value) => {
     ...
     const validateForm = () => {
        ...
        if (formData.conditionType === 'DAYS_OVERDUE' && (formData.conditionValueInteger === '' || isNaN(conditionInt) || conditionInt < 0)) {
            errors.conditionValueInteger = 'Please enter a valid number of days (0 or more).';
        }
        const conditionDec = Number(formData.conditionValueDecimal);
         if (formData.conditionType === 'MIN_AMOUNT_DUE' && (formData.conditionValueDecimal === '' || isNaN(conditionDec) || conditionDec < 0)) {
            errors.conditionValueDecimal = 'Please enter a valid minimum amount (0 or more).';
        }
    ...
    return (
        <Dialog.Root open={isOpen} onOpenChange={onClose}>
            <Dialog.Content style={{ maxWidth: 550 }}>
                ...
                <Dialog.Description>
                    {isEditing ? `Editing "${rule?.ruleName || ''}"` : 'Define a new rule for automated actions.'}
                </Dialog.Description>
                    ...
                    <Flex direction="column" gap="4" mt="4">
                        ...
                        <FormRow label="Rule Name" htmlFor="ruleName" error={formErrors.ruleName}>
                            <input
                                ...
                                placeholder="e.g., 'Initial 7-Day Reminder'"
                                required
                        ...
                        {formData.conditionType === 'DAYS_OVERDUE' && (
                            <FormRow label="Days Overdue Is Greater Than" htmlFor="conditionValueInteger" error={formErrors.conditionValueInteger}>
                                <input
                        ...
                        {formData.actionType === 'SEND_EMAIL' && (
                            <FormRow label="Notification Template" htmlFor="templateId" error={formErrors.templateId}>
                                ...
                                >
                                    <option value="" disabled={formData.templateId !== ""}>Select a template...</option>
                                    {templates.length > 0 ? (
                                        templates.map(template => (
                                            <option key={template.templateId} value={template.templateId}>
                                                {template.templateName} ({template.templateId.substring(0, 8)}...)
                                            </option>
                                        ))
                                    ) : (
