// in: src/features/customer/pages/StatusPage.jsx
...
import PaymentHistoryModal from '../components/PaymentHistoryModal'; // <-- Import Modal
import { CalendarIcon, CheckCircledIcon, CrossCircledIcon } from '@radix-ui/react-icons';

const StatusPage = () => {
    // 1. Get ALL props from the hook, including payment-related ones
    const {
        profile,
        accounts,
        loading,
        ...
        payments,           // <-- Need this
        paymentLoading,     // <-- Need this
        paymentError,       // <-- Need this
        fetchPaymentsForInvoice // <-- Need this
    } = useCustomerData();
    ...
    const handleViewPayments = (invoice) => {
        ...
        setIsModalOpen(true);
        fetchPaymentsForInvoice(invoice.invoiceId); // Fetch payments when modal opens
    };
    ...
    // IMPORTANT: Make sure the 'currentBalance' fix (explained below) is done in the backend
    const totalBalance = accounts.reduce((sum, acc) => sum + (acc.currentBalance || 0), 0);
    // Ensure the status string exactly matches your backend enum ('OVERDUE')
    const overdueInvoices = invoices.filter(inv => inv.status === 'OVERDUE').length;

    return (
        // Wrap in Fragment to allow sibling Modal component
        <>
            <Flex direction="column" gap="6">
                        ...
                        <Flex direction="column" gap="2">
                            <Flex align="center" gap="2">
                                <div className="w-8 h-8 bg-green-500 rounded-full flex items-center justify-center">
                                    {/* Using a simple dollar sign icon */}
                ...
                <InvoiceTable
                    ...
                    onViewPayments={handleViewPayments} // <-- Pass handler down
                />
            </Flex>
