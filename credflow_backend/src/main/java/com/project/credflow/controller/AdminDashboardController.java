...
import com.project.credflow.repository.PaymentRepository; // Import PaymentRepository
...
public class AdminDashboardController {
    ...
    /**
     * Endpoint to manually trigger the dunning process.
     */
    @PostMapping("/run-engine")
    public ResponseEntity<Map<String, String>> manuallyRunDunningEngine() {
        try {
            // We run this async so the API returns immediately
            // (Requires @EnableAsync on main app & @Async on the service method)
    ...
    public ResponseEntity<Map<String, Object>> getLiveStats() {
        ...
        long totalOverdueCount = invoiceRepository.countByStatus("OVERDUE");
        stats.put("totalOverdueCount", totalOverdueCount);

        // 2. Total Overdue Amount ($)
        BigDecimal totalOverdueAmount = invoiceRepository.sumAmountDueByStatus("OVERDUE");
        ...
        stats.put("pendingTasks", 0); // Placeholder until BPO is built

        // 4. Last Run Time
        Optional<LocalDateTime> lastRunTime = logRepository.findFirstByOrderByExecutedAtDesc()
                .map(log -> log.getExecutedAt());
        stats.put("lastRunTime", lastRunTime.orElse(null)); // Will be null if never run
    ...
    public ResponseEntity<Map<String, Object>> getKpisByDate(
        ...
        LocalDateTime endDateTime = endDate.plusDays(1).atStartOfDay(); // Up to (but not including) the next day
